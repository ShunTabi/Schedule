...
namespace Crane
{
    class ConSQL
    {
        ...
        {
            public static string SQL0300 = $"SELECT * FROM V_WORK WHERE GOALNAME LIKE @KEYWORD OR PLANNAME LIKE @KEYWORD ORDER BY PRIORID,WORKSTARTDATE,WORKENDDATE LIMIT {ComSQL.SQLLimit}";
        ...
        public class AnalysisSQL
        {
            ...
            public static string SQL0502 = $"SELECT DISTINCT MONTH FROM V_ST_SCHEDULE2 WHERE MONTH BETWEEN @MONTH1 AND @MONTH2 LIMIT {ComSQL.SQLLimit}";
            //public static string SQL0502 = $"SELECT DISTINCT MONTH FROM V_ST_MONTH LEFT OUTER JOIN (SELECT DISTINCT GOALID,GOALNAME FROM V_SCHEDULE) WHERE MONTH BETWEEN @MONTH1 AND @MONTH2 ORDER BY MONTH LIMIT {ComSQL.SQLLimit}";
            public static string SQL0503 = $"SELECT DISTINCT MONTH FROM V_ST_SCHEDULE2 WHERE MINS !=0 AND MONTH BETWEEN @MONTH1 AND @MONTH2 AND GOALID = @GOALID  GROUP BY MONTH LIMIT {ComSQL.SQLLimit}";
