...
INNER JOIN T_GOAL USING(GOALID)
INNER JOIN V_STATUS ON T_WORK.PRIORID = V_PRIOR.PRIORID
...
INNER JOIN T_PLAN USING(PLANID)
INNER JOIN T_GOAL USING(GOALID)
INNER JOIN V_STATUS ON T_REVIEW.STATUSID = V_STATUS.STATUSID
WHERE GOALVISIBLESTATUS = 0 AND PLANVISIBLESTATUS = 0 AND REVIEWVISIBLESTATUS = 0;
-- V_SCHEDULE
...
-- V_STORAGEBIN
CREATE VIEW V_STORAGEBIN AS
SELECT 'GENRE' AS "KEY",GENREID AS "ID",GENRENAME AS "NAME",GENREUPDATEDATE AS "UPDATEDATE" FROM T_GENRE WHERE GENREVISIBLESTATUS = 2
UNION
SELECT 'GOAL',GOALID,GOALNAME,GOALUPDATEDATE FROM T_GOAL WHERE GOALVISIBLESTATUS = 2
UNION
SELECT 'PLAN',PLANID,PLANNAME,PLANUPDATEDATE FROM T_PLAN WHERE PLANVISIBLESTATUS = 2
UNION
SELECT 'WORK',WORKID,PLANNAME||'・'||WORKNAME,WORKUPDATEDATE FROM T_WORK INNER JOIN T_PLAN USING(PLANID)
WHERE WORKVISIBLESTATUS = 2
UNION
SELECT 'REVIEW',REVIEWID,PLANNAME||'・'||REVIEWNAME,REVIEWUPDATEDATE FROM T_REVIEW INNER JOIN T_PLAN USING(PLANID)
WHERE REVIEWVISIBLESTATUS = 2
UNION
...
WHERE SCHEDULEVISIBLESTATUS = 2;
-- V_RECYCLEBIN
CREATE VIEW V_RECYCLEBIN AS
